...
jso <- filter(cec17.final, Algorithm == "jSO", Dimension == 30) %>%
    select(Result) %>% unlist()
ebo.cmar <- filter(cec17.final, Algorithm == "EBO-CMAR", Dimension == 30) %>%
    select(Result) %>% unlist()
bst.results <- rNPBST::bayesianSign.test(ebo.cmar, jso,
                                       rope.min = -10, rope.max = 10)
bsrt.results <- rNPBST::bayesianSignedRank.test(ebo.cmar, jso,
                                              rope.min = -10, rope.max = 10)
```
...
plotSimplex(bst.results$sample, num.points = 10000) 
plotSimplex(bsrt.results$sample, num.points = 10000)
```
...
plot(bayesian.imprecise.results)
```
