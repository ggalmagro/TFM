...
In order to carry out this task we will make use the widely known frequentist non parametric two-tailed Wilcoxon test, as well as the lesser-known Bayesian sign test.

\subsection{Applying Wilcoxon test to our results}

\textcolor{red}{Buscar a alguien que sepa explicarme que cojones es lo que hace el Keel. Las hipótesis no pueden ser las que he planteado y problemente el alpha no se use para calcular el estadístico T}

We will apply the Wilcoxon test for each of the measures obtained as a result, namely: ARI, Unsat and Time. Remember that the Wilcoxon test is a hypothesis test, whose hypotheses are established as:

\begin{itemize}
	
	\item $H_0$: differences between pairs of results follow a symmetric distribution around zero.
	
	\item $H_1$: differences between pairs of results do not follow a symmetric distribution around zero.
	
\end{itemize}

It is also necessary to establish a confidence level to obtain the statistical values that will allow us to make a decision about $H_0$. In this case we consider a confidence level of 95\%, which means that the value of $\alpha$ for the calculation of the statistic $T$ will be $\alpha = 1 - 0.95 = 0.05$. Table \ref{tab:freqTest} shows the statistical values obtained using Wilcoxon test for each comparison.
...
\begin{table}[!htp]
	...
	\begin{tabular}{cccc}
		\hline
		VS & $R^{+}$ & $R^{-}$ & P-value \\ \hline 
		ARI & 3203.5 & 1746.5 & 0.006936\\
		Time & 4008.0 & 1042.0 & 0\\  
		Unsat & 3829.5 & 1120.5 & 0.000002\\ \hline 
	\end{tabular}
	\caption{Results obtained by the Wilcoxon test for algorithm SHADE (vs BRKGA)}
	\label{tab:freqTest}
\end{table}

It is worth noting that, in the case of ARI, this is a maximization problem, while in the remaining two measures we try to minimize.

The results are clear, in all cases $p-value < \alpha$, which means that we can reject the null hypothesis. We conclude that there are significant differences between the two methods in favour of SHADE+LS.
