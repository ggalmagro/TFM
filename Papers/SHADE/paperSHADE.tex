...
Genetic Algorithms (GA) emerged as one of the first metaheuristics inspired by the concept of natural selection and evolution. It is one of the most studied and successful evolutionary algorithms thanks in part to its simplicity and easy implementation. \cite{fausto2019ants}

In GA a Population $P$ of a given number of solutions $p_i = \{p_{[i,1]}, \cdots, p_{[i,v]}\}$ is initialized, these solutions are also called chromosomes or individuals. Each element of a chromosome is called gene and the value it takes is known as allele. To simulate the natural processes of evolution and natural selections the GA uses a set of evolutionary operator similar to the one used by DE, this is: crossover, mutation and selection. The crossover operator consist of combining two different chromosomes (parents) from the population and obtaining a new chromosome (offspring) which inherit alleles for its genes from both parents. The mutation operator is used by GA to avoid local optimum, it consist of introducing random changes in the population to favor diversity. The selection operator is uses to determine if a chromosome is suitable to survive for the next generation, to accomplish this task it uses a fitness function, which assign a value to each chromosome in order to determine if it is a good solution to the problem or not. GA applies these operator through a series of generations to hopefully obtain a solution for the problem to solve close to the optimal. \cite{fausto2019ants}

GAs have been applied to all kinds of problems, including problems whose solutions are subject to strong restrictions such as machine scheduling, resource location and the quadratic assignment problem. These problems present the peculiarity that, by applying generalized crossover operators to two individuals that represent a feasible solution to the problem, we can obtain one that is not feasible. The Random-Key Genetic Algorithm (RKGA) emerged in \cite{bean1994genetic} as a solid approach to this kind of problems, it uses random numbers to represent solutions to the problem (individuals), which are used as keys to obtain an actual solutions. Random-key vectors are sampled from the space $[0,1]^{v}$, which is used as a surrogate space for the literal solutions space. Points in the random-key space are transfered to the literal solutions space via a decoder for fitness evaluation. A decoder is a deterministic procedure that allow us to map random-key vectors to the literal solutions space. We can build a decoder that always provides as a result a feasible solution to the problem we want to solve, thus avoiding the feasibility problem that we found in the classic GA.
...
It should be noted that in BRKGA there is a clear differentiation between problem-dependent and problem-independent elements. The independent part of the algorithm has no knowledge of the problem it solves, it is limited to performing the operations related to the population optimization process. The dependent part of the problem is the decoder and the fitness function, which allow us to obtain real solutions for the problem and its evaluation. Therefore, to specify a BRKGA optimization scheme it is only necessary to define the decoder and the fitness function. \cite{gonccalves2011biased}
...
\section{Adaptation of SHADE for constrained clustering} \label{sec:SHADEadapt}
...
\begin{algorithm}
	...
	\While{Termination criteria are not met}{
		...
		\tcp{Whenever $|A| > |P|$, randomly select an individual from $A$ to be deleted so that $|A| \le |P|$}
