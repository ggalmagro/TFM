...
def main():
    ...
    general_table_file = open("Results/ISA_general_table_file.txt", "w+")
    results_file = open("Results/ISA_results_file.txt", "w+")
    ...
    for label_percent in range(len(const_percent_vector)):
        ...
        for i in range(len(names_array)):
            ...
            for j in range(nb_runs):
                ...
                isa = ISA(data_set, ml_const, cl_const, nb_clust, 0.3, 300, 0.3, 0.4, 0.001)
                start = time.time()
                isa_assignment = isa.run(max_eval)[1]
                end = time.time()
                mean_ars += adjusted_rand_score(labels, isa_assignment)
                mean_execution_time += end - start
                mean_unsat_percent += get_usat_percent(ml_const, cl_const, isa_assignment)
    ...
    #
    # iris_plot3 = draw_data_2DNC(iris_set, np.asarray(iris_isa_assignment, np.float), 3, alg + "Iris Dataset Results ML")
    # iris_plot4 = draw_data_2DNC(iris_set, np.asarray(iris_isa_assignment, np.float), 3, alg + "Iris Dataset Results CL")
    #
